...
import Uploader from "../components/ulpoader";
...
function Table(props) {
  ...
  const handleData = (data) => {
    console.log("coming from Uploader", data);
    setCookie(data);
    setnewCookies(data);
    setCookiePresent(true);
    setIsLoading(false);
  ...
  const sendNewCookies = () => {
    ...
    for (let i = 0; i < cookie.length; i++) {
      ...
      //making a payload
      const payLoad = {
        cookie_name: cookie[i].cookie_name,
        cookie_domain: cookie[i].cookie_domain,
        cookie_id: cookie[i].cookie_id,
        cookie_value: cookie[i].cookie_value.slice(0, 20),
        cookie_expiry_date: cookie[i].cookie_expiration_date,
      };
